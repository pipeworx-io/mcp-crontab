...
`tools/list` at `https://gateway.pipeworx.io/crontab/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/describe_cron \
  -H 'Content-Type: application/json' \
  -d '{"expression":"0 9 * * 1-5"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/describe_cron`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
